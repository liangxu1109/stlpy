...
class STLTree(STLFormula):
    ...
    """
    def __init__(self, subformula_list, combination_type, timesteps, robustness_type, name=None):
    ...
    def robustness(self, y, t):
        if self.combination_type == "and":
            out =[]
            if self.robustness_type == "AGM":
                if isinstance(self.subformula_list[0], STLTree) && isinstance(self.subformula_list[1], ):
                    right = self.subformula_list[1].robustness
                    out.append(right)
                left = self.subformula_list[0].robustness
                return out
            elif self.robustness_type == "Traditional":
                return min([formula.robustness(y, t+self.timesteps[i]) for i, formula in enumerate(self.subformula_list)])
        else: # combination_type == "or"
            if self.robustness_type == "AGM":
                out = []
                array = np.array([formula.robustness(y, t+self.timesteps[i]) for i, formula in enumerate(self.subformula_list)])
